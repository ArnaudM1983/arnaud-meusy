      ...
      <Link href={`/work/${slug}`} className="block group">
        ...
        <div className="mt-6 flex flex-col md:flex-row justify-between items-start gap-4">
          <div className="transition-all duration-500">
            <h3 className="text-2xl md:text-3xl font-sans font-bold text-[#1A1A1A] uppercase tracking-tighter">
              {title}
            </h3>
            <p className="text-lg md:text-xl text-[#4A4A4A] group-hover:text-[#1A1A1A] mt-1 transition-colors duration-300">
              {impact}
            </p>
          </div>

          <div className="hidden md:block h-px bg-[#1A1A1A]  flex-grow mx-8 mt-4 group-hover:opacity-40 transition-all duration-500" />

          <ArrowUpRight className="text-[#FF6006] w-8 h-8 self-end md:self-auto group-hover:translate-x-2 group-hover:-translate-y-2 transition-transform duration-300" />
        </div>
      </Link>
    </motion.div>
  );
